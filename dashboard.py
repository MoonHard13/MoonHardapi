...
class ClientDashboard(ctk.CTk):
    def __init__(self):
        ...
        self.send_btn = ctk.CTkButton(self.buttons_frame, text="📤 Send get_status", command=self.send_status)
        ...
        # === Επιλογή πελάτη ===
        self.selected_client = ctk.CTkEntry(self, placeholder_text="Enter client_id")
        self.selected_client.pack(pady=10)
    ...
    def send_status(self):
        client_id = self.selected_client.get().strip()
        if not client_id:
            self.status_box.insert("end", "⚠️ Please enter a client ID\n")
            return
        try:
            response = requests.post(
                f"{SERVER_URL}/send/{client_id}?token={AUTH_TOKEN}",
                json={"command": "get_status"}
            )
            self.status_box.insert("end", f"📤 Sent to {client_id}: {response.json()}\n")
        except Exception as e:
            self.status_box.insert("end", f"❌ Error sending: {e}\n")
